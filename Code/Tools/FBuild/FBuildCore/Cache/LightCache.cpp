                       ...
                       Array< AString > & outIncludes )
{
    ...
    outSourceHash = FBuild::Hash64( hashes.Begin(), hashes.GetSize() * sizeof( uint64_t ) );
...
void LightCache::Parse( IncludedFile * file, FileStream & f )
{
    ...
    // Store hash of file
    file->m_ContentHash = FBuild::Hash64( fileContents );
...
{
    const uint64_t fileNameHash = FBuild::Hash64( fileName );
