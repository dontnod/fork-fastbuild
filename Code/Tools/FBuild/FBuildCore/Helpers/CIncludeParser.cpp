...
//------------------------------------------------------------------------------
CIncludeParser::CIncludeParser(const AString& rootPath)
    : m_RootPath(rootPath)
    , m_LastCRC1( 0 )
    , m_CRCs1( 4096, true )
    , m_LastCRC2( 0 )
...
void CIncludeParser::AddInclude( const char * begin, const char * end )
{
    ...
    // quick check
    uint32_t crc1 = FBuild::Hash32( m_RootPath, begin, (size_t)( end - begin ) );
    if ( crc1 == m_LastCRC1 )
    {
        return;
    }
    ...
    NodeGraph::CleanPath( include, cleanInclude );
        ...
        lowerCopy.ToLower();
        uint32_t crc2 = FBuild::Hash32( m_RootPath, lowerCopy );
    #else
        // Linux is case-sensitive
        uint32_t crc2 = FBuild::Hash32( m_RootPath, cleanInclude );
    #endif
    if ( crc2 == m_LastCRC2 )
    {
        return;
